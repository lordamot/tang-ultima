...
static void spi_task(void *parms) {
  ...
  while(1) {
    ...
    // While ultima.c moves the FPGA to another core the link is dead
    // for a few seconds and then belongs to a core that has not been set
    // up: whatever the interrupt line does meanwhile is not an event.
    // A read now would take floating MISO for requests - a "write" to a
    // mounted image among them - so nothing is read until the hold is
    // lifted; the line is looked at again a little later.
    if(sys_irq_hold) {
