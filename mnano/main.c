...
static void osd_task(void *parms) {
  ...
  // Tang Ultima: the card may ask for another core than the one that
  // came up (slot 0, or the one the OSD's switch left us on).  Get
  // there before anything is set up for the wrong machine.
  int timeout = 200;
  while(timeout && !sdc_is_ready()) {
    vTaskDelay(pdMS_TO_TICKS(10));
    timeout--;
  }
  if(timeout) ultima_boot(spi);
  else        printf("SD wasn't ready, staying on the core that came up\r\n");
...
int main(void) {
  ...
  // try to establish connection to FPGA for ten seconds. Assume the FPGA is
  // not properly configured after that.  (Ten, not MiSTeryNano's five: after
  // the OSD's core switch the MCU restarts while the FPGA is still loading
  // the next image, which takes about three seconds by itself - ultima.c.)
  int fpga_ok, timeout = 1000;
  do {
  ...
  if(timeout) {
    ...
    printf("FPGA not ready after 10 seconds!\r\n");
